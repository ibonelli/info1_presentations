...
int conectar (int argc, char * argv[])
{
	...
	int	sockfd;
	struct hostent *he;	/* Se utiliza para convertir el nombre del host a su direcciÃ³n IP */
	struct sockaddr_in their_addr;  /* direcciÃ³n del server donde se conectarÃ¡ */

	/* Convertimos el nombre del host a su direcciÃ³n IP */
...
int Open_conection ( struct sockaddr_in * my_addr)
{
	...
	/*Crea un socket y verifica si hubo algÃºn error*/
	if ((sockaux = socket(AF_INET, SOCK_STREAM, 0)) == -1) 
	{
		fprintf(stderr, "Error en funciÃ³n socket. CÃ³digo de error %s\n", strerror(sockaux));
		return -1;
	}
...
/* Asignamos valores a la estructura my_addr */
	...
	/* Con la estructura sockaddr_in completa, se declara en el Sistema que este proceso escucharÃ¡ pedidos por la IP y el port definidos*/
	if ( (aux = bind (sockaux, (struct sockaddr *) my_addr, sizeof(struct sockaddr))) == -1)
	{
		fprintf(stderr, "Error en funciÃ³n bind. CÃ³digo de error %s\n", strerror(aux));
		return -1;
	}
	/* Habilitamos el socket para recibir conexiones, con una cola de conexiones en espera que tendrÃ¡ como mÃ¡ximo el tamaÃ±o especificado en BACKLOG*/
...
int Aceptar_pedidos (int sockfd)
{
	...
	{
		printf  ("server:  conexiÃ³n desde:  %s\n", inet_ntoa(their_addr.sin_addr));
		return newfd;
	}
}
